...
import { Menu, X, ArrowRight, Grid2x2 as Grid, List, ChevronLeft, ChevronRight, Phone as PhoneIcon, MapPin } from 'lucide-react';
...
const ProductsPage: React.FC<ProductsPageProps> = ({ 
  ...
  const categories = ['All Products', 'Clear Series', 'Color Series'];
...
const closeProductModal = () => {
  ...
  const nextImage = () => {
    if (selectedProduct) {
      setCurrentImageIndex((prev) => (prev + 1) % 4); // Assuming 4 images per product
    }
  };
  ...
  // Handle ESC key to close modal and search
  React.useEffect(() => {
    ...
    <div className="min-h-screen bg-black">
      ...
      <div className="pt-20">
        ...
        <div className="py-12 text-center">
          <h1 className="text-5xl md:text-6xl font-bold text-white mb-4">Products</h1>
          <p className="text-xl text-gray-300 max-w-3xl mx-auto px-4">
            Discover our premium collection of architectural glass blocks, designed to transform spaces with light, texture, and unparalleled craftsmanship.
          </p>
        </div>
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {categories.map((category) => (
                <button
                  ...
                  style={selectedCategory === category ? { backgroundColor: '#D7B387' } : {}}
                >
                  {category}
                </button>
              ))}
...
<div className="flex items-center gap-2">
  ...
  {/* List button - hidden on mobile, visible on md+ */}
  <button
          ...
          {viewMode === 'grid' ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {filteredProducts.map((product) => (
                ...
                >
                  ...
                  <div className="p-4">
                    <h3 className="text-white font-semibold mb-1">{product.code}</h3>
                    <p className="text-gray-400 text-sm mb-2">{product.name}</p>
                    <p className="text-xs opacity-75" style={{ color: '#D7B387' }}>
                      {product.category}
                    </p>
                    <button className="mt-3 w-full text-black py-2 px-4 rounded font-medium hover:opacity-90 transition-opacity text-sm"
            ...
            <div className="space-y-4">
              {filteredProducts.map((product) => (
                <div
                  key={product.id}
                  className="bg-gray-900 rounded-lg p-6 hover:bg-gray-800 transition-colors group cursor-pointer flex items-center gap-6"
                  onClick={() => openProductModal(product)}
                >
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm opacity-75" style={{ color: '#D7B387' }}>
                      {product.category}
                    </p>
                    <p className="text-gray-300 text-sm mt-2">{product.description}</p>
                  </div>
                  <button className="text-black py-2 px-6 rounded font-medium hover:opacity-90 transition-opacity"
                          style={{ backgroundColor: '#D7B387' }}>
                    View Details
                  </button>
                </div>
          ...
          {filteredProducts.length === 0 && (
            <div className="text-center py-12">
              ...
              <button
                ...
                className="mt-4 text-white hover:opacity-80 transition-opacity"
                style={{ color: '#D7B387' }}
              >
                Clear filters
              </button>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg max-w-4xl w-full max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center p-6 border-b">
              <div>
                <h2 className="text-2xl font-bold text-gray-900">{selectedProduct.code}</h2>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div>
                  <div className="relative aspect-square mb-4 bg-gray-100 rounded-lg overflow-hidden">
                    <img
                      src={(selectedProduct.images || [selectedProduct.image])[currentImageIndex]}
                      alt={selectedProduct.name}
                      className="w-full h-full object-cover"
                    />
                    <div className="absolute top-4 right-4 bg-black bg-opacity-50 text-white px-2 py-1 rounded text-sm">
                      {currentImageIndex + 1} / {(selectedProduct.images || [selectedProduct.image]).length}
                    </div>
                    {(selectedProduct.images || [selectedProduct.image]).length > 1 && (
                      <>
                        <button
                          onClick={() => setCurrentImageIndex(currentImageIndex > 0 ? currentImageIndex - 1 : (selectedProduct.images || [selectedProduct.image]).length - 1)}
                          className="absolute left-4 top-1/2 transform -translate-y-1/2 bg-black bg-opacity-50 text-white p-2 rounded-full hover:bg-opacity-70 transition-all"
                        >
                          <ChevronLeft className="h-5 w-5" />
                        </button>
                        <button
                          onClick={() => setCurrentImageIndex(currentImageIndex < (selectedProduct.images || [selectedProduct.image]).length - 1 ? currentImageIndex + 1 : 0)}
                ...
                <div>
                  ...
                  {/* Long Description */}
                  <div className="mb-6">
                    <h3 className="text-lg font-semibold text-gray-900 mb-2">Long Description</h3>
                  ...
                  {/* Contact Button */}
                    ...
                    onClick={() => setShowContactModal(true)}
                       ...
                       className="w-full px-4 py-3 border border-gray-600 bg-black text-white rounded-lg focus:outline-none focus:ring-2 focus:ring-[#D7B387] focus:border-[#D7B387]"      
                  >
                    Contact for Quote
                  </button>
                </div>
          ...
          <div className="bg-white rounded-lg max-w-md w-full">
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="flex items-start space-x-4">
                ...
                <div>
                  <h3 className="font-semibold text-gray-900 mb-2">Calling Hours</h3>
